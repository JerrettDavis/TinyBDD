...
{
    var methods =
        from kind  in new[]{ Kind.Transform, Kind.Effect }
        from title in Titles
        from async in Asyncs
        from token in Tokens
        where TrySig(Step.When, kind, async, token, out var _)
        let sig = (TrySig(Step.When, kind, async, token, out var s), s).s
        select MakeWhen(kind, title, async, token, sig);
...
void Emit_AndBut()
{
    string Build(string wordName, string wordEnum)
    {
        var methods =
            ...
            from async in Asyncs
            from token in Tokens
            where TrySig(Step.And, kind, async, token, out var _)
            let sig = (TrySig(Step.And, kind, async, token, out var s), s).s
            select MakeAndOrBut(wordName, wordEnum, kind, title, async, token, sig);
...
void Emit_Then()
{
    ...
    var preds =
        ...
        from async in Asyncs
        from token in Tokens
        where TrySig(Step.Then, Kind.Predicate, async, token, out var _)
        let sig = (TrySig(Step.Then, Kind.Predicate, async, token, out var s), s).s
        select MakeThenPredicate(title, async, token, sig);

    // no-value predicates (sync + Task only)
    var predsNoValue =
        from title in Titles
        from async in new[]{ Async.Sync, Async.Task }
        select MakeThenPredicateNoValue(title, async);

    // value-based asserts
    var asserts =
        from title in Titles
        from async in new[]{ Async.Task, Async.ValueTask }
        from token in Tokens
        where TrySig(Step.Then, Kind.Assert, async, token, out var _)
        let sig = (TrySig(Step.Then, Kind.Assert, async, token, out var s), s).s
        select MakeThenAssert(title, async, token, sig);

    // no-value asserts (no token-aware form in public surface)
    var assertsNoValue =
        from title in Titles
        from async in new[]{ Async.Task, Async.ValueTask }
        select MakeThenAssertNoValue(title, async);

    // assertion transform returning TOut (Task only; token aware + unaware)
    var assertOut =
        from title in Titles
        from token in Tokens
        let sig = token == Token.None ? "Func<T, Task<TOut>>" : "Func<T, CancellationToken, Task<TOut>>"
        select MakeThenAssertTransformOut(title, token, sig);
