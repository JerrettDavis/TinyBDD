...
string RenderExtension(MethodSpec m, string chainType)
{
    ...
    // Add T to generics if needed (for extension methods that use T in the this parameter)
    var generics = m.Generics;
    if (!string.IsNullOrEmpty(generics))
    {
        // If we have <TOut>, change to <T, TOut>
        if (generics == "<TOut>")
        {
            generics = "<T, TOut>";
        }
        // If we have other generics without T, add T
