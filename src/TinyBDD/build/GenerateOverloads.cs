...
void Emit_AndBut()
{
    string BuildForAsync(string wordName, string wordEnum, Async @async)
    {
        var methods =
            new[] { Kind.Transform, Kind.Effect }
                .SelectMany(kind => Titles, (kind, title) => new { kind, title })
                .SelectMany(k => new[] { @async }, (k, a) => new { k.kind, k.title, @async = a })
                .SelectMany(k => Tokens, (k, token) =>
                {
    ...
    var file =
        "// <auto-generated/>\nnamespace TinyBDD;\npublic sealed partial class ScenarioChain<T>\n{\n" +
        BuildForAsync("And", "StepWord.And", Async.Sync) +
        BuildForAsync("And", "StepWord.And", Async.Task) +
        BuildForAsync("And", "StepWord.And", Async.ValueTask) +
        BuildForAsync("But", "StepWord.But", Async.Sync) +
        BuildForAsync("But", "StepWord.But", Async.Task) +
        BuildForAsync("But", "StepWord.But", Async.ValueTask) +
        "}\n";
...
void Emit_Then()
{
    ...
    var file =
        ...
        string.Concat(syncPredsNoValue.Select(Render)) +
        string.Concat(actionAsserts.Select(Render)) +
        string.Concat(actionAssertsNoValue.Select(Render)) +
        string.Concat(taskPreds.Select(Render)) +
        string.Concat(taskPredsNoValue.Select(Render)) +
        string.Concat(taskAsserts.Select(Render)) +
        string.Concat(taskAssertsNoValue.Select(Render)) +
        string.Concat(assertOut.Select(Render)) +
        string.Concat(valueTaskPreds.Select(Render)) +
        string.Concat(valueTaskAsserts.Select(Render)) +
        string.Concat(valueTaskAssertsNoValue.Select(Render)) +
        "}\n";
...
bool BddMethodUsesValueTask(BddGenMethod m) =>
    m.Params.Any(p => p.Type.Contains("ValueTask"));
...
{{
{string.Concat(syncMethods.Select(m => BddGen_Render(m, false)))}}}
";
...
/// <summary>
/// Generated Task-based Given(...) overloads for Bdd.
/// </summary>
public static partial class Bdd
{{
{string.Concat(taskMethods.Select(m => BddGen_Render(m, false)))}}}
";

  var valueTaskContent =
$@"// <auto-generated/>
using System.Threading;
using System.Threading.Tasks;
...
public static partial class Bdd
{{
{string.Concat(valueTaskMethods.Select(m => BddGen_Render(m, false)))}}}
";

  var syncPath = Path.Combine(projectDir, "Generated", "Bdd.Given.g.cs");
  var taskPath = Path.Combine(projectDir, "Generated", "Bdd.Given.Tasks.g.cs");
  var valueTaskPath = Path.Combine(projectDir, "Generated", "Bdd.Given.ValueTasks.g.cs");
  
  File.WriteAllText(syncPath, syncContent);
  File.WriteAllText(taskPath, taskContent);
  File.WriteAllText(valueTaskPath, valueTaskContent);
  
  Console.WriteLine($@"Wrote {syncPath}");
  Console.WriteLine($@"Wrote {taskPath}");
...
MethodSpec MakeFinally(Title title, Async @async, Token token, string sig) =>
    new(
        $"Registers a cleanup handler {(title==Title.Explicit ? "with an explicit title" : "")} that executes after all steps complete.\n    /// The handler receives the current chain value and executes even if subsequent steps fail.",
        Xml(
            title == Title.Explicit ? "/// <param name=\"title\">Display title for this finally handler.</param>" : null,
        ...
        "Finally",
        "",
        Params(title, sig, Kind.Effect, explicitName: "effect"),
        $"FinallyEffect({TitleArg(title)}, ToCT(effect))"
    );

MethodSpec MakeFinallyThenChain(Title title, Async @async, Token token, string sig) =>
    new(
        $"Registers a cleanup handler {(title==Title.Explicit ? "with an explicit title" : "")} that executes after all steps complete.\n    /// The handler receives the current chain value and executes even if subsequent steps fail.",
        Xml(
            title == Title.Explicit ? "/// <param name=\"title\">Display title for this finally handler.</param>" : null,
            "/// <param name=\"effect\">Cleanup action that receives the carried value.</param>",
            "/// <returns>The same <see cref=\"ThenChain{T}\"/> for further chaining.</returns>"
        ),
        "ThenChain<T>",
        "Finally",
        "",
...
void Emit_Finally()
{
    var methods =
        Titles
            ...
            .SelectMany(k => Tokens, (k, token) =>
            {
                var ok = TrySig(Step.When, Kind.Effect, k.@async, token, out var sig);
                return new { k.title, k.@async, token, ok, sig };
            })
            .Where(x => x.ok)
            .Select(x => MakeFinally(x.title, x.@async, x.token, x.sig));
    ...
    File.WriteAllText(Path.Combine(outDir, "ScenarioChain.Finally.g.cs"), file);
}

void Emit_Finally_ThenChain()
{
    var methods =
        Titles
            .SelectMany(title => Asyncs, (title, @async) => new { title, @async })
            ...
            .Where(x => x.ok)
            .Select(x => MakeFinallyThenChain(x.title, x.@async, x.token, x.sig));
