...
public class GivenAndButPermutationsTests(ITestOutputHelper output) : TinyBddXunitBase(output)
{
    [Scenario("Given -> And -> When -> Then (sync transforms)")]
    [Fact]
    public async Task Given_And_When_Then_Sync()
    {
        await Flow.Given("seed 1", () => 1)          // 1
            .And("+2", x => x + 2)                   // 3
            .When("x3", x => x * 3)                  // 9
            .Then("== 9", v => v == 9);
    ...
    {
        await Flow.Given("seed 10", () => 10)                    // 10
